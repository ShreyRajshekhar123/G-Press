...
export default function NewsCard({
  ...
  const handleBookmarkToggle = async (event) => {
    ...
    try {
      ...
      if (isBookmarked) {
        ...
        const deleteUrl = `http://localhost:5000/api/news/bookmark/${bookmarkToDelete._id}`;
        ...
        response = await axios.post(
          "http://localhost:5000/api/news/bookmark",
          payload,
          { headers: headers }
        );
  ...
  const handleGenerateQuestions = async (event) => {
    ...
    try {
      ...
      const url = `http://localhost:5000/api/questions/generate-on-demand/${sourceKeyForURL}/${displayNews._id}`;
